...
mongoose.connect('mongodb://' + mongo_config.username + ':' + mongo_config.password + '@'+ mongo_config.host + ':' + mongo_config.port + '/' + mongo_config.database);
...
function onSuccess(from_currency, to_currency, callback) {
	...
	Job.findOneAndUpdate({from_currency: from_currency, to_currency: to_currency}, {$inc:{success:1}, error:0}, {upsert: true, 'new': true}, function(err, job){
		if (err) {
			callback(err);
		} else {
...
function onError(from_currency, to_currency, callback) {
	Job.findOneAndUpdate({from_currency: from_currency, to_currency: to_currency}, {$inc:{error:1}}, {upsert: true, 'new': true}, function(err, job){
		if (err) {
			callback(err);
		} else {
...
function removeJob(from_currency, to_currency, callback) {
	...
	Job.remove({from_currency: from_currency, to_currency: to_currency}, function(err) {
		callback(err);
	});
}
